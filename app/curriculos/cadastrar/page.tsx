...
const validarMesAnoPassado = (value?: string | null) => {
  if (!value || value.length < 7) return true; // Deixa o "min" ou "required" cuidar se estiver vazio
  const [mes, ano] = value.split('/');
  const numMes = parseInt(mes, 10);
  const numAno = parseInt(ano, 10);

  if (numMes < 1 || numMes > 12) return false; // Bloqueia mês 13, 14, etc.

  const hoje = new Date();
  const mesAtual = hoje.getMonth() + 1; // getMonth() vai de 0 a 11 no JS
...
export default function CadastrarCurriculo() {
  ...
  const onSubmit = async (data: any) => {
    ...
    localStorage.setItem("@CresceCV:curriculos", JSON.stringify([...curriculosAtuais, novoCurriculo]));

    toast.success("Currículo salvo com sucesso!", {
      description: `O currículo de ${data.nome} ${data.sobrenome} foi armazenado localmente.`,
    });
