...
// Interface para resolver os erros de tipagem do VS Code
...
function EditarFormContent() {
  ...
  useEffect(() => {
    async function carregarDados() {
      ...
      try {
        const encontrado = await curriculoService.getById(idDaUrl) as any;
        
        if (encontrado) {
          const experienciasMapeadas = encontrado.experiencias?.map((exp: Experiencia) => ({
            ...exp,
        ...
        console.error(error);
        toast.error("Erro ao carregar currículo para edição.");
      } finally {
        setCarregandoIniciais(false);
      }
    }
  ...
  const onSubmit = async (data: any) => {
    if (!idDaUrl) return;
    try {
      await curriculoService.update(idDaUrl, data);
      toast.success("Currículo atualizado com sucesso!");
      router.push(`/curriculos/visualizar/detalhes?id=${idDaUrl}`);
    } catch (error) {
      console.error(error);
...
export default function EditarCurriculo() {
  return (
    <Suspense fallback={<div className="p-8 max-w-4xl mx-auto text-center"><Skeleton className="h-96 w-full" /></div>}>
      <EditarFormContent />
    </Suspense>
  );
}